...
const Post = ({ socket, streamUpdated, setStreamUpdated }) => {
	...
	return (
		<div className="w-full min-h-full overflow-scroll grid grid-cols-12 relative pl-2">
			<CSSTransition
				nodeRef={overlayRef}
				in={showOverlay}
				timeout={300}
			...
			<div className="col-span-8 border-r-2 border-gray-200 flex flex-col overflow-y-auto pt-2 pr-2 relative w-full">
				<button
					onClick={() => {
						setPostLoading(true);
				...
				<div
					className={`h-2/3 overflow-y-auto mb-3 pr-2 flex flex-col-reverse underlineLink ${
				...
				>
					...
					{postLoading ? (
						...
						posts
							.sort((a, b) => {
								return a.postData.createdAt <
									b.postData.createdAt
									? 1
									: -1;
							})
							.map((post, index) => {
				...
				<form
					className="h-1/3 border border-solid border-b border-gray-200 flex flex-col items-center justify-start"
			...
			<div className="col-span-4 p-4 flex flex-col justify-between">
				...
				{isHidden || (
					<form
						className="group fixed bottom-3"
